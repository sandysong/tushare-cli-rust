---
name: tushare-cli
description: 使用 Tushare CLI 工具获取中国金融市场数据。当用户需要获取股票、指数、基金、期货、债券等金融数据时使用此 skill。支持 238 个 Tushare Pro 数据接口，包括股票行情、财务数据、宏观经济指标等。使用零依赖的命令行工具直接调用 HTTP API，无需 Python SDK。
---
...
```bash
# 获取股票列表
./skill/scripts/tushare stock_basic --list-status L --format markdown
...
**JSON** - 默认格式，适合程序处理：
```bash
./skill/scripts/tushare stock_basic --format json
```

**Markdown** - 推荐格式，适合在对话中展示：
```bash
./skill/scripts/tushare stock_basic --format markdown
```
...
**CSV** - 适合导入到 Excel：
```bash
./skill/scripts/tushare stock_basic --format csv
```

**Table** - 终端友好显示：
```bash
./skill/scripts/tushare stock_basic --format table
```
...
# 搜索相关接口
./skill/scripts/tushare search 股票
./skill/scripts/tushare search 基金
...
```bash
# 获取指数列表
./skill/scripts/tushare index_basic --market SSE --format markdown
...
```bash
# 获取基金列表
./skill/scripts/tushare fund_basic --market E --format markdown
...
3. 或在命令中使用 `--token` 参数：
   ```bash
   ./skill/scripts/tushare stock_basic --token YOUR_TOKEN
   ```
